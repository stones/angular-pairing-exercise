...
export class RegionAgentsListComponent implements OnInit {
  ...
  getRegionAgents() {
    this.dataService.getRegionAgents(this.regionId).subscribe((agents) => this.agents = agents);
  }
  ...
  getShortAmount(amount, decimalPlaces){
    ...
    k = amount / 1e15;	if (k >= 1) return shorten(k, decimalPlaces, "Q");
    k = amount / 1e12;	if (k >= 1) return shorten(k, decimalPlaces, "T");
    k = amount / 1e9;	if (k >= 1) return shorten(k, decimalPlaces, "B");
    k = amount / 1e6;	if (k >= 1) return shorten(k, decimalPlaces, "M");
    k = amount / 1e3;	if (k >= 1) return shorten(k, decimalPlaces, "K");
  }
